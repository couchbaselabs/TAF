...
class CollectionBase(ClusterSetup):
    ...
    def create_sdk_clients(cluster, num_threads, servers_to_connect,
                           buckets, sdk_compression, load_using):
                ...
                for bucket in buckets_spec["buckets"]:
                    ...
                    if buckets_spec["buckets"][bucket].get(Bucket.bucketType,
                        ...
                        buckets_spec["buckets"][bucket][
                            ...
                            test_obj.encryptionAtRestDekRotationInterval
                        test_obj.log.info(
                            "Encryption at rest enabled for bucket: %s" % bucket)
                    else:
                        buckets_spec["buckets"][bucket][
    ...
    def create_clients_for_sdk_pool(test_obj, cluster=None):
        if test_obj.load_docs_using not in ["default_loader",
                                            "sirius_java_sdk"]:
