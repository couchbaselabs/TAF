...
from Columnar.onprem.columnar_onprem_base import OnPremBase
...
class CopyToKV(OnPremBase):
    ...
    def build_cbas_columnar_infra(self):
        ...
        self.columnar_spec["dataverse"]["no_of_dataverses"] = self.input.param("no_of_dataverses", 1)
        self.columnar_spec["databases"]["no_of_databases"] =  self.input.param("no_of_databases", 1)
        self.columnar_spec["remote_link"]["no_of_remote_links"] = self.input.param("no_of_remote_links", 1)
        ...
        self.columnar_spec["remote_dataset"]["no_of_remote_datasets"] = self.input.param("no_of_remote_datasets", 1)
        self.columnar_spec["standalone_dataset"]["num_of_standalone_coll"] = self.input.param(
    ...
    def test_copy_to_kv_onprem(self):
        ...
        remote_link = self.columnar_cbas_utils.get_all_link_objs("couchbase")[0]
        jobs = Queue()
        results = []
        ...
        self.columnar_cbas_utils.run_jobs_in_parallel(jobs, results, self.sdk_client_per_user, async_run=False)
        remote_bucket = self.remote_cluster.buckets[0]
        scope = self.remote_cluster.buckets[0].scopes["_default"]
        collections = self.remote_cluster.buckets[0].scopes["_default"].collections
        for i,key in enumerate(collections):
            collection = "{}.{}.{}".format(self.columnar_cbas_utils.format_name(remote_bucket.name), scope.name,
                                           collections[key].name)
            jobs.put((self.columnar_cbas_utils.copy_to_kv,
                      {"cluster": self.cluster, "collection_name": datasets[i].name, "database_name": datasets[i].database_name,
                       "dataverse_name": datasets[i].dataverse_name, "dest_bucket": collection,
                       "link_name": remote_link.full_name}))

        self.columnar_cbas_utils.run_jobs_in_parallel(jobs, results, self.sdk_client_per_user)
        if not all(results):
            self.fail("Copy to kv statement failed")

        for i,key in enumerate(collections):
            remote_dataset = self.columnar_cbas_utils.create_remote_dataset_obj(self.cluster,remote_bucket.name,
                                                                      scope.name, collections[key].name, remote_link,
                                                                      capella_as_source=True)[0]
            if not self.columnar_cbas_utils.create_remote_dataset(self.cluster, remote_dataset.name,
            ...
            columnar_count = self.columnar_cbas_utils.get_num_items_in_cbas_dataset(self.cluster, datasets[i].full_name)
            kv_count = self.columnar_cbas_utils.get_num_items_in_cbas_dataset(self.cluster, remote_dataset.full_name)
            if columnar_count != kv_count:
                self.log.error("Doc count mismatch in KV and columnar {0}, {1}, expected: {2} got: {3}".
                               format(collections[key], datasets[i].full_name,
        ...
        for i, key in enumerate(collections):
            ...
            jobs.put((self.columnar_cbas_utils.copy_to_kv,
                      {"cluster": self.cluster, "collection_name": datasets[i].name,
                       ...
                       "dataverse_name": datasets[i].dataverse_name, "dest_bucket": collection,
                       "link_name": remote_link.full_name, "validate_error_msg": True,
                       "expected_error": self.input.param("expected_error"),
                       "expected_error_code": self.input.param("expected_error_code", None)}))

        self.columnar_cbas_utils.run_jobs_in_parallel(jobs, results, self.sdk_client_per_user, async_run=True)
        time.sleep(20)
        if self.input.param("kill_memcached", False):
            for i in range(5):
                self.kill_memcached(self.remote_cluster, self.input.param("kill_master", False),
    ...
    def test_rebalance_in_remote_copy_to_kv(self):
        ...
        rebalance_util = CBASRebalanceUtil(self.cluster_util, self.bucket_util, self.task, vbucket_check=False)

        if rebalance_stage == "before":
            rebalance_task, self.available_servers = rebalance_util.rebalance(self.remote_cluster, kv_nodes_in=1,
                                                                              available_servers=self.available_servers)
            results.append(rebalance_util.wait_for_rebalance_task_to_complete(rebalance_task, self.remote_cluster))
            self.columnar_cbas_utils.run_jobs_in_parallel(jobs, results, self.sdk_client_per_user, async_run=False)
    ...
    def test_rebalance_out_remote_copy_to_kv(self):
        ...
        if rebalance_stage == "before":
            rebalance_task, self.available_servers = rebalance_util.rebalance(self.remote_cluster, kv_nodes_out=1,
    ...
    def test_failover_remote_copy_to_kv(self):
        ...
        if rebalance_stage == "before":
            available_servers, kv_failover_nodes, _ = rebalance_util.failover(self.remote_cluster, kv_nodes=1,
        ...
        if rebalance_stage == "during":
            jobs.put((rebalance_util.failover,
                      {"cluster": self.remote_cluster, "kv_nodes": 1,
                       "failover_type": graceful, "exclude_noes": nodes_to_keep}))
            self.columnar_cbas_utils.run_jobs_in_parallel(jobs, results, self.sdk_client_per_user, async_run=False)
