...
class ColumnarRBAC(ColumnarOnPremBase):
    ...
    def setUp(self):
        ...
        self.link_connection_privileges = ["CONNECT", "DISCONNECT", "DESCRIBE"]
    ...
    def generate_test_case(self, user, privileges, resources, validate_err_msg=False,
        ...
        test_case['predefined_roles'] = predefined_roles
    ...
    def create_rbac_testcases(self, privileges=[], resources=[], privilege_resource_type="database",
        ...
        table_view.set_headers(["User", "Password", "Role", "Privilege", "Resource Type", "Resource"])
        ...
        table_view.add_row([ea_admin_user.name, ea_admin_password, "analytics_admin", "all", "all", "all"])
        ...
        for priv in privileges:
            ...
            table_view.add_row([user1.name, password, "", priv, privilege_resource_type, resources])
            if not result:
            ...
            self.columnar_rbac_util.assign_role_to_user(self.analytics_cluster,
                                                        roles=[role1],
                                                        users=[user2])
            table_view.add_row([user2.name, password, role1.role_name, priv, privilege_resource_type, resources])
        ...
        result = self.columnar_rbac_util.grant_privileges_to_subjects(self.analytics_cluster,
                                                                      ...
                                                                      resources,
                                                                      privilege_resource_type)
        table_view.add_row([user3.name, password, "", privileges, privilege_resource_type, resources])
        if not result:
        ...
        result = self.columnar_rbac_util.assign_role_to_user(self.analytics_cluster,
                                                             roles=[role2],
                                                             users=[user4])
        table_view.add_row([user4.name, password, role2.role_name, privileges, privilege_resource_type, resources])
        if not result:
        ...
        table_view.add_row([user5.name, password, "", "", privilege_resource_type, resources])
        ...
        result = self.columnar_rbac_util.assign_role_to_user(self.analytics_cluster,
                                                             [role3],
                                                             [user6])
        table_view.add_row([user6.name, password, role3.role_name, "", privilege_resource_type, resources])
        if not result:
        ...
        for pre_def_role in self.predefined_roles:
            ...
            result = self.columnar_rbac_util.assign_role_to_user(self.analytics_cluster,
                                                                 [pre_def_role],
                                                                 [created_user])
            table_view.add_row([created_user.name, password, pre_def_role, "", privilege_resource_type, resources])
            if not result:
    ...
    def test_rbac_collection_ddl(self):
        ...
        for idx, test_case in enumerate(testcases):
            ...
            result = self.columnar_rbac_util.grant_privileges_to_subjects(
                        ...
                        ["CREATE COLLECTION"], [test_case['user']],
                        [],
                        privilege_resource_type="link_dml",
                        use_any=True)
            if not result:
                self.fail("Failed to assing CREATE COLLECTION privilege to user {} on links".
                        format(str(test_case['user'])))
            ...
            for priv in privileges:
                for res in resources:
                    ...
                    if priv == "INSERT":
                        ...
                        predefined_roles = test_case['predefined_roles']
        ...
        for idx, test_case in enumerate(testcases):
            ...
            for priv in privileges:
                if priv == "CREATE":
                    ...
                    expected_error = self.ACCESS_DENIED_ERR.format("LINK", remote_link_obj.name, priv)
                    ...
                    expected_error = self.ACCESS_DENIED_ERR.format("LINK", s3_link_obj.name, priv)
                    ...
                    result = self.columnar_cbas_utils.create_external_link(self.analytics_cluster,
                                                                           s3_link_obj.properties)
                    if not result:
                        self.fail("Failed to create external link {}".format(s3_link_obj.name))
            ...
            for priv in privileges:
                if priv == "COPY TO":
                    result = self.columnar_rbac_util.grant_privileges_to_subjects(
                        ...
                        privilege_resource_type="collection_dml")
                    if not result:
                        self.fail("Failed to assign SELECT privilege to user {} on collection {}". \
        ...
        for idx, test_case in enumerate(testcases):
            ...
            for priv in privileges:
                ...
                if priv == "CREATE":
                    ...
                    result = self.columnar_rbac_util.drop_columnar_role(
                        self.analytics_cluster,
                        role_name,
                        validate_error_msg=test_case['validate_err_msg'],
                        expected_error=expected_error,
