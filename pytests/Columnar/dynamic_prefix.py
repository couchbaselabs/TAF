...
class DynamicPrefix(ColumnarBaseTest):
    ...
    def test_create_external_collections_with_dynamic_prefixes(self):
        ...
        for testcase in testcases:
            try:
                ...
                self.cbas_util.create_external_link_from_spec(
                    self.cluster, self.columnar_spec)
                ...
                for dataset_obj in self.cbas_util.get_all_dataset_objs():
                    if not self.cbas_util.create_dataset_on_external_resource(
                            ...
                            dataset_obj.dataset_properties["timezone"],
                            True, False, None, testcase["expected_error"],
                            None):
    ...
    def test_create_query_external_collections_with_mismatch_in_actual_and_expected_path(self):
        ...
        self.columnar_spec["external_dataset"]["num_of_external_datasets"] \
            = self.input.param("num_of_external_datasets", 1)
        ...
        for result in results:
            if result != 0:
                self.fail("Doc count mismatch. Expected - {0}, Actual - {1}"
                          .format(0, result[0]))
        ...
        for result in results:
            if result[0] != "success":
                ...
                self.fail("Doc count mismatch. Expected - 0, Actual - {0}"
                          .format(len(result[3])))
        ...
        if not self.skip_item_count_check:
            ...
            for result in results:
                if result != self.doc_count_per_format[file_format]:
                    self.fail("Doc count mismatch. Expected: {0}, Actual: {1}"
                              .format(self.doc_count_per_format[file_format],
                                      result))
