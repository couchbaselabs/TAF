...
class basic_ops(BaseTestCase):
    ...
    def __chunks(self, list_chunk, n):
        """Yield successive n-sized chunks from list_chunk."""
        for i in range(0, len(list_chunk), n):
            yield list_chunk[i:i + n]
    ...
    def __thread_to_transaction(self, transaction, op_type, doc, txn_commit,
                                update_count=1, sync=True, set_exception=True,
                                client=None):
        exception = None
        if not client:
            client = self.client
    ...
    def test_MultiThreadTxnLoad(self):
        """ Load data through txn, update half the items through
        different threads and delete half the items through different threads.
        If update_retry then update and delete the same key in two different
        transaction and make sure update fails"""
    ...
    def test_basic_retry_async(self):
        ...
        thread = threading.Thread(
            ...
            args=(self.transaction, "create", self.docs,
                  self.transaction_commit, self.update_count, True, False))
        thread.start()
        self.sleep(1)
    ...
    def test_stop_loading(self):
        ...
        for doc in docs:
            threads.append(threading.Thread(
                target=self.__thread_to_transaction,
                args=(self.transaction, "create", doc, self.transaction_commit,
        ...
        self.create_Transaction(self.client1)
        self.sleep(self.transaction_timeout+60,
                   "Wait for transaction to cleanup")
