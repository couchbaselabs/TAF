...
class UserAdminRole(ClusterSetup):
    ...
    def tearDown(self):
        self._cleanup_rbac_test_artifacts()
        super(UserAdminRole, self).tearDown()

    def _cleanup_rbac_test_artifacts(self):
        """Best-effort removal of the RBAC users/groups this test class creates.
        Several test methods intentionally leave some behind (verify_error
        deletes are expected to be Forbidden for 3 of the 4 security_users),
        and the cluster isn't reset between test cases, so without this state
        leaks across runs (confirmed via stray users on a shared test cluster).
        """
        try:
            users = self.rest.retrieve_user_roles()
        except Exception as err:
            self.log.warning("tearDown: failed to list users for cleanup: {}".format(err))
            users = []
        ...
        for user in users:
            ...
            if not user_id:
                continue
            if user_id not in self.security_users and not user_id.startswith("security_user_"):
                continue
            try:
                if user.get("domain") == "external":
                    self.rest.delete_external_user(user_id)
                else:
                    self.rest.delete_builtin_user(user_id)
            except Exception as err:
                self.log.warning("tearDown: failed to delete user {}: {}".format(user_id, err))

        for group_name in ["security_group_1", "security_group_2",
                           "security_group_3", "security_group_4"]:
            try:
    ...
    def list_users(self, username=None, verify_error=False):
        ...
        try:
            ...
            # GET /settings/rbac/users does not deny user_admin_local/
            # user_admin_external outright when they lack the other domain's
            # list permission - it silently domain-scopes the response instead
            # (confirmed against a live cluster). So verify_error here means
            # "expect a domain-filtered list", not "expect an empty one" - the
            # per-user domain check below is the real access-control assertion.
    ...
    def _raise_if_forbidden(self, resp):
        # delete_builtin_user/add_set_bulitin_group/delete_builtin_group return
        # the error body instead of raising on a non-2xx response, unlike every
        # other RestConnection RBAC method; surface it the same way those do.
        if isinstance(resp, dict) and self.expected_error in str(resp.get("message", "")):
    ...
    def test_privilege_escalation(self):
        ...
        # Test that both local and external admin users cannot change their own roles to admin roles.
        # Non-admin roles (e.g. query_external_access) are not included: user_admin roles are
        # allowed to grant those to other users, so self-modification with them is consistent.
        roles = admin_roles
