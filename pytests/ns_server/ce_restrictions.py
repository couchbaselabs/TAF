...
def _get_stats_settings(rest):
    """
    GET /settings/stats
    Returns usage stats configuration (includes sendStats flag).

    This branch's cb_server_rest_util submodule pin predates
    ClusterRestAPI.get_stats_settings()/set_stats_settings(), so both are
    reimplemented here directly against the generic request() helper.
    """
    api = rest.base_url + "/settings/stats"
    headers = rest.create_headers(rest.username, rest.password)
    status, content, _ = rest.request(api, "GET", headers=headers)
    return status, content


def _set_stats_settings(rest, send_stats):
    """
    POST /settings/stats
    ...
    headers = rest.create_headers(rest.username, rest.password)
    status, content, _ = rest.request(api, "POST", params, headers)
    return status, content
...
class CommunityEditionRestrictions(ClusterSetup):
    ...
    def test_ce_phonehome_enforcement(self):
        ...
        status, settings = _get_stats_settings(self.rest)
        ...
        status, content = _set_stats_settings(self.rest, send_stats=False)
        ...
        if cli_rejected:
            ...
            if not status:
                ...
                ok, s = _get_stats_settings(rest_init)
            ...
            rest_init = ClusterRestAPI(master)
            ok, settings = _get_stats_settings(rest_init)
