...
class RebalanceDurability(RebalanceBaseTest):
    ...
    def test_replica_update_with_durability_without_adding_removing_nodes(self):
        ...
        for replicas in range(2, 4):
            self.log.info("Updating the bucket replicas to {0}".format(replicas))
            self.bucket_util.update_all_bucket_replicas(replicas=replicas)
            rebalance = self.task.rebalance(self.cluster.servers[:self.nodes_init], [], [])
    ...
    def test_replica_update_with_durability_with_adding_removing_nodes(self):
        ...
        for replicas in range(2, 4):
            self.log.info("Increasing the bucket replicas to {0}".format(replicas))
            self.bucket_util.update_all_bucket_replicas(replicas=replicas)
            rebalance = self.task.rebalance(self.cluster.nodes_in_cluster, [self.cluster.servers[replicas]], [])
        ...
        for replicas in range(3, 0):
            self.log.info("Reducing the bucket replicas to {0}".format((replicas - 1)))
