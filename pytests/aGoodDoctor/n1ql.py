...
HotelIndexes = ['CREATE INDEX {}{} ON {}(country, DISTINCT ARRAY `r`.`ratings`.`Check in / front desk` FOR r in `reviews` END,array_count((`public_likes`)),array_count((`reviews`)) DESC,`type`,`phone`,`price`,`email`,`address`,`name`,`url`) PARTITION BY HASH (country) USING GSI WITH {{ "defer_build": true, "num_replica":1, "num_partition":8}};',
                ...
                'CREATE INDEX {}{} ON {}(ALL ARRAY `r`.`ratings`.`Rooms` FOR r IN `reviews` END,`avg_rating`)  PARTITION BY HASH (avg_rating) USING GSI WITH {{ "defer_build": true, "num_replica":1, "num_partition":8}};',
                'CREATE INDEX {}{} ON {}(`city`) PARTITION BY HASH (city) USING GSI WITH {{ "defer_build": true, "num_partition":8}};',
                'CREATE INDEX {}{} ON {}(`price`,`name`,`city`,`country`) PARTITION BY HASH (name) USING GSI WITH {{ "defer_build": true, "num_partition":8}};',
                'CREATE INDEX {}{} ON {}(`name` INCLUDE MISSING DESC,`phone`,`type`) PARTITION BY HASH (name) USING GSI WITH {{ "defer_build": true, "num_partition":8}};',
                'CREATE INDEX {}{} ON {}(`city` INCLUDE MISSING ASC, `phone`) PARTITION BY HASH (city) USING GSI WITH {{ "defer_build": true, "num_partition":8}};',
...
def execute_via_sdk(client, statement, readonly=False, client_context_id=None, query_params=None, validate=True):
    params = dict()
    if query_params:
        for k, v in query_params.items():
            try:
                params.update({k: eval(v)})
            except:
                params.update({k: v})
    options = QueryOptions(metrics=True,
...
def performKNNSearchES(esClient, collection, qVec, k, gt, es_params=None):
    try:
        esResult = esClient.search(
            index=collection.lower().replace("_", ""),
            body={
                "query": {
                    "knn": {
                        "field": "embedding",
        ...
        print(e)
        esClient = Elasticsearch(esClient.transport.hosts)
        esResult = esClient.search(
            index=collection.lower().replace("_", ""),
            body={
                "query": {
    ...
    esResult = esResult.to_dict()
    recall = 0
    all_ids_elastic = [hit["_source"]["id"] for hit in esResult["hits"]["hits"]]
        ...
        for b in buckets:
            ...
            for s in self.bucket_util.get_active_scopes(b, only_names=True):
                ...
                if b.name+s not in self.sdkClients.keys():
                    self.sdkClients.update({b.name+s: b.clients[0].cluster})
                    time.sleep(1)
                collections = list(b.scopes[s].collections.keys())
                    ...
                    while i < workload.get("2i")[0] or q < workload.get("2i")[1]:
                        ...
                        if q < workload.get("2i")[1]:
                            ...
                            if workload.get("vector"):
                                ...
                                query = unformatted_q.format(bucket=b.name, scope=s, collection=c,
                                                             similarity=similarity, nProbe=nProbe)
                            else:
                                query = unformatted_q.format(bucket=b.name, scope=s, collection=c)
                            print (query)
                ...
                for bucket in _buckets:
                    ...
                    for collection in sorted(d.keys())[i:i+build_count]:
                        ...
                        build_query = "BUILD INDEX on %s.%s.%s(%s) USING GSI" % (
                            bucket.name, s, collection, ",".join(sorted(d.get(collection))))
                        # time.sleep(1)
                        start = time.time()
                        while time.time() < start + 600:
                            try:
                                ...
                                break
                            except Exception or InternalServerFailureException as e:
                                raise(e)
                if wait:
                    for bucket in _buckets:
                        d = defaultdict(list)
    ...
    def _run_vector_query(self, validate_item_count=True, expected_count=100):
        ...
        while not self.stop_run:
            ...
            try:
                ...
                self.query_stats[query][0] += metrics.execution_time().total_seconds()*1000.0
        ...
        while not self.stop_run:
            ...
            try:
                ...
                status, metrics, _, results, _ = execute_statement_on_n1ql(
                    self.cluster_conn, query, client_context_id,
                    q_param, validate=validate_item_count)
                exec_time = metrics.execution_time().total_seconds()*1000.0
