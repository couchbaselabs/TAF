...
class RRGuardrails(GuardrailsBase):
    ...
    def test_rr_guardrail_with_add_nodes(self):
        ...
        if self.rebalance_pause:
            self.sleep(20)
            reached = self.cluster_util.rebalance_reached(self.cluster,
                                                          self.reb_pause_per)
            if reached:
                stopped = rebalance_util.stop_rebalance(wait_timeout=self.wait_timeout / 3)
    ...
    def initial_data_load_until_guardrail_limit(self, guardrail_limit, create_start=0, create_end=2500000):
        ...
        # Start the load without waiting so we can poll the resident ratio and
