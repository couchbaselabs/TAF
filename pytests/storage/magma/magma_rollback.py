    ...
    def test_magma_rollback_with_large_docs(self):
        ...
        while upsert_iterations > 0:
            self.log.info("Upserting docs. Iteration: {}"
                          .format(total_iter - upsert_iterations + 1))
            self.load_data_cbc_pillowfight(self.cluster.master, bucket,
        ...
        self.log.info(f"Vbuckets on the master node = {self.target_vbucket}")
        ...
        task = self.task.async_load_gen_docs(
            self.cluster, bucket, gen_create, DocLoading.Bucket.DocOps.CREATE,
            timeout_secs=self.sdk_timeout,
        ...
        vb_dict1 = self.bucket_util.get_vb_details_for_bucket(
            bucket, self.cluster.nodes_in_cluster)
        ...
        for vbucket in range(self.vbuckets):
            ...
            if vbucket in self.target_vbucket:
                self.assertTrue(
                    vb_active_seq > vb_active_seq_prev,
                    f"High seqno of the active target vb {vbucket_name} "
        ...
        for vbucket in range(self.vbuckets):
            ...
            for j in range(len(replica_list)):
                ...
                if vbucket in self.target_vbucket:
                    self.assertTrue(
                        vb_replica_seq > vb_replica_seq_prev,
                        f"High seqno of the replica target vb {vbucket_name} "
                        "has not been incremented")
        ...
        self.sleep(20, "Wait before fetching vbucket-details")
        vb_dict2 = self.bucket_util.get_vb_details_for_bucket(
            bucket, self.cluster.nodes_in_cluster)
            ...
            for j in range(len(replica_list)):
                ...
                if vbucket in self.target_vbucket:
                    self.assertTrue(
                        vb_replica_seq == vb_replica_seq_prev,
                        f"High seqno of the replica target vb {vbucket_name} "
                        f"does not match after rollback")
        shell.disconnect()
