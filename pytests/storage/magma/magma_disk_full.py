    ...
    def test_disk_full_with_large_docs(self):
        ...
        for i in range(2000):
            ...
            if res["error"] is not None:
                self.log.info("Insert failed for key: {0}. Error = {1}".format(key, res["error"]))
            else:
        ...
        if read_on_disk_full:
            ...
            for prefix in doc_key_prefixes:
                for i in range(20):
                    ...
                    try:
                        res = sdk_client2.read(doc_key, timeout=30)
                    except Exception as e:
                        self.log.info("Exception while reading doc:{0} = {1}".format(doc_key, e))
        ...
        self.load_data_cbc_pillowfight(self.cluster.master, bucket, 2500, 2548000, "2mbbig_docs3")
        self.load_data_cbc_pillowfight(self.cluster.master, bucket, 500, 16548000, "16mbbig_docs3")
        self.load_data_cbc_pillowfight(self.cluster.master, bucket, 500, 20548000, "20mbbig_docs3")
