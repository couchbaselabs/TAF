...
class SecurityUpgradeTests(UpgradeBase):
    ...
    def _update_users_during_upgrade(self, scenario):
        # Note: with nodes_init=3/nodes_upgrade=3 (all conf entries), this is only ever
        # invoked for "Node 1 Upgrade" and "Node 2 Upgrade" -- there is no node left to
        # upgrade after the 3rd node, so a "Node 3 Upgrade" scenario is never reached.
        user_roles = {
    ...
    PERMISSION_SET = ["cluster.admin.security!all", "cluster.admin.users!all",
                      "cluster.admin.security!read", "cluster.admin.users!read"]

    def _check_permissions_for_user(self, user):
        status, resp = self.rbac_util._check_user_permissions(
            user, "password", self.PERMISSION_SET)
        self.assertTrue(status,
                        msg="checkPermissions call failed for user {0}: {1}".format(user, resp))
        return resp
