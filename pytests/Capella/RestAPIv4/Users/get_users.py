...
class GetUsers(PostUser):

    def setUp(self, nomenclature="User_Get"):
        PostUser.setUp(self, nomenclature)

    def tearDown(self):
        super(GetUsers, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.fetch_user_info(
                organization, user)
    ...
    def test_authorization(self):
        ...
        for testcase in self.v4_RBAC_injection_init([
            ...
            result = self.capellaAPI.cluster_ops_apis.fetch_user_info(
                self.organisation_id, self.user_id, header)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.fetch_user_info(
                    self.organisation_id, self.user_id, header)
