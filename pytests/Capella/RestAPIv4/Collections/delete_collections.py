...
class DeleteCollection(APIBase):
    ...
    def setUp(self):
        ...
        self.project_id = self.capellaAPI.org_ops_apis.create_project(
            self.organisation_id, self.prefix + "CollectionsDelete",
            self.generate_random_string(
                100, prefix=self.prefix)).json()["id"]

        # Initialize params for cluster creation.
        cluster_name = self.prefix + "TestCollectionsDelete"
        cluster = {
            "name": cluster_name,
            "description": None,
            "cloudProvider": {
        ...
        bucket_name = "TestCollectionsDelete"
        bucket = {
            "name": bucket_name,
            "type": "couchbase",
        ...
        for testcase in testcases:
            ...
            if result.status_code == 200:
                if "expected_error" in testcase:
                    ...
                    self.scope_name = self.create_collection_to_be_tested(
                        self.organisation_id, self.project_id,
