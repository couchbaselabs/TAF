...
class GetCollection(GetScope):

    def setUp(self, nomenclature="Collections_Get"):
        GetScope.setUp(self, nomenclature)

        # Create a collection inside the scope and create expected result.
        self.collection_name = self.prefix + nomenclature
        self.expected_res = {
            "maxTTL": 0,
    ...
    def tearDown(self):
        ...
        if self.capellaAPI.cluster_ops_apis.delete_collection(
                ...
                self.collection_name).status_code != 204:
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.fetch_collection_info,
            (self.organisation_id, self.project_id, self.cluster_id,
             self.bucket_id, self.scope_name, self.collection_name)
        ]]
