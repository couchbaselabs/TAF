...
class DeleteClusterSchedule(GetClusterSchedule):

    def setUp(self, nomenclature="Clusters_Schedule_Delete"):
        GetClusterSchedule.setUp(self, nomenclature)

    def tearDown(self):
        super(DeleteClusterSchedule, self).tearDown()

    def test_api_path(self):
        testcases = [
            {
                "description": "Delete valid schedule"
            }, {
        ...
        for testcase in testcases:
            ...
            if self.validate_testcase(res, [204], testcase, failures):
                self.log.info("Deletion successful")
                time.sleep(2)
                res = self.capellaAPI.cluster_ops_apis.\
                    create_cluster_on_off_schedule(
                        self.organisation_id, self.project_id,
                        self.cluster_id, self.expected_result["timezone"],
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.delete_cluster_on_off_schedule,
            (self.organisation_id, self.project_id, self.cluster_id,
             "", self.expected_result["days"])
        ]]
