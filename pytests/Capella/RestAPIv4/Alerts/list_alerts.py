...
class ListAlert(GetAlert):

    def setUp(self, nomenclature="Alerts_List"):
        GetAlert.setUp(self, nomenclature)

        # Instantiate expected result for List endpoint
        self.expected_res = {
            "cursor": {
                "hrefs": {
                    "first": None,
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.list_alerts(org, proj)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.list_alerts(
                    org, proj)
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[self.capellaAPI.cluster_ops_apis.list_alerts,
                          (self.organisation_id, self.project_id)]]
