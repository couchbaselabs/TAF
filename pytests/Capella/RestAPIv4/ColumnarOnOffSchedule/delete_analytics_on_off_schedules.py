...
class DeleteOnOffSchedule(GetOnOffSchedule):

    def setUp(self, nomenclature="ColumnarScheduleOnOff_DELETE"):
        GetOnOffSchedule.setUp(self, nomenclature)

    def tearDown(self):
        super(DeleteOnOffSchedule, self).tearDown()
    ...
    def test_query_parameters(self):
        ...
        for combination in self.create_path_combinations(
            ...
            result = self.columnarAPI.delete_on_off_schedule(
                ...
                testcase["analyticsClusterID"], **kwarg)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.columnarAPI.delete_on_off_schedule(
                    testcase["organizationID"], testcase["projectID"],
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.columnarAPI.delete_on_off_schedule, (
