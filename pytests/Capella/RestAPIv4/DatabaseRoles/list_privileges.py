...
class ListCapellaPrivileges(DatabaseRoleBase):

    def setUp(self, nomenclature="DatabaseRoles_Privileges_LIST"):
        DatabaseRoleBase.setUp(self, nomenclature)

    def tearDown(self):
        super(ListCapellaPrivileges, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            if "url" in testcase:
                self.capellaAPI.cluster_ops_apis.db_privileges_endpoint = \
                    testcase["url"]
            ...
            result = self.capellaAPI.cluster_ops_apis.list_capella_privileges(
                organization, project, cluster)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = \
                    self.capellaAPI.cluster_ops_apis.list_capella_privileges(
                        organization, project, cluster)
            self.capellaAPI.cluster_ops_apis.db_privileges_endpoint = \
                "/v4/organizations/{}/projects/{}/clusters/{}/privileges"
