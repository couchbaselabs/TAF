...
class DeleteCluster(GetProject):

    def setUp(self, nomenclature="Clusters_Delete"):
    ...
    def test_api_path(self):
        testcases = [
            {
                "description": "Delete a dummy cluster",
                ...
                "invalid_clusterID": self.replace_last_character(
                    self.dummy_cluster_id, non_hex=True),
                "expected_status_code": 400,
                "expected_error": {
                    "code": 1000,
                    "hint": "Check if all the required params are present "
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.delete_cluster(
                org, proj, clus)
            ...
            if result.status_code == 404:
                if isinstance(testcase["expected_error"], dict) \
                    ...
                    self.log.debug("This is a handler condition for the dummy "
                                   "cluster deletion request, we expect a "
                                   "{}, but as the clusterID is dummy, the "
                                   "deletion will return a 404:4025.".format(
                                    testcase["expected_error"]))
            else:
                self.validate_testcase(result, 404, testcase, failures)
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[self.capellaAPI.cluster_ops_apis.delete_cluster,
                          (self.organisation_id, self.project_id,
                           self.dummy_cluster_id)]]
