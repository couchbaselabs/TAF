...
class GetAIWorkflow(AIWorkflowBase):

    def setUp(self, nomenclature="AIWorkflows_GET"):
        AIWorkflowBase.setUp(self, nomenclature)

    def tearDown(self):
        super(GetAIWorkflow, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            if "url" in testcase:
                self.capellaAPI.cluster_ops_apis.ai_workflow_endpoint = \
                    testcase["url"]
            ...
            result = self.capellaAPI.cluster_ops_apis.get_ai_workflow(
        ...
        for combination in self.create_path_combinations(
            ...
            if not (combination[0] == self.organisation_id and
                ...
                if (combination[0] == "" or combination[1] == "" or
                    ...
                    testcase["expected_error"] = {
                        "code": 404,
                        "hint": "The requested workflow details could not be "
                                "found or fetched. Please ensure that the "
                                "correct workflow ID is provided.",
            ...
            result = self.capellaAPI.cluster_ops_apis.get_ai_workflow(
                ...
                testcase["clusterID"], testcase["workflowID"], **kwarg)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.get_ai_workflow(
                    testcase["organizationID"], testcase["projectID"],
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.get_ai_workflow, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.workflow_id
            )
        ]]
...
class DeleteAIWorkflow(AIWorkflowBase):

    def setUp(self, nomenclature="AIWorkflows_DELETE"):
        AIWorkflowBase.setUp(self, nomenclature)

    def tearDown(self):
        super(DeleteAIWorkflow, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.delete_ai_workflow(
                organization, project, cluster, workflow)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.delete_ai_workflow(
            ...
            if self.validate_testcase(result, [204], testcase, failures):
                self.log.debug("Deletion Successful, recreating workflow.")
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.delete_ai_workflow, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.workflow_id
            )
        ]]
