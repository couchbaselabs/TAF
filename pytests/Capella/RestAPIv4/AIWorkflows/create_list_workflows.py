...
class CreateAIWorkflow(AIWorkflowBase):

    def setUp(self, nomenclature="AIWorkflows_POST"):
        AIWorkflowBase.setUp(self, nomenclature)

    def tearDown(self):
        super(CreateAIWorkflow, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            if "url" in testcase:
                self.capellaAPI.cluster_ops_apis.ai_workflows_endpoint = \
                    testcase["url"]
            ...
            result = self.capellaAPI.cluster_ops_apis.create_ai_workflow(
    ...
    def test_payload(self):
                ...
                if key == "name":
                    if value is None or value == "":
                        ...
                        testcase["expected_error"] = {
                            ...
                            "httpStatusCode": 400,
                            "message": "Bad Request. Error: workflow name is "
                                       "required."
                        }
        ...
        for testcase in testcases:
            ...
            payload = {k: testcase[k] for k in self.expected_res.keys()}
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.create_ai_workflow, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.expected_res
            )
        ]]
...
class ListAIWorkflows(AIWorkflowBase):

    def setUp(self, nomenclature="AIWorkflows_LIST"):
        AIWorkflowBase.setUp(self, nomenclature)
        self.expected_res = {
            "cursor": {
                "hrefs": {
    ...
    def tearDown(self):
        super(ListAIWorkflows, self).tearDown()
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.list_ai_workflows, (
                self.organisation_id, self.project_id, self.cluster_id
            )
        ]]
        self.throttle_test(api_func_list)
