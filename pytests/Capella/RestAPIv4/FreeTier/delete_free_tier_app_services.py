...
class DeleteFreeTier(GetFreeTierApp):

    def setUp(self, nomenclature="FreeTier_DELETE"):
        GetFreeTierApp.setUp(self, nomenclature)

        # Insuring App Service is healthy to be deleted.
        start_time = time.time()
        while not self.validate_onoff_state(
                ["healthy"], app=self.free_tier_app_id,
                free_tier=self.free_tier_cluster_id):
            if time.time() > 1800 + start_time:
                self.tearDown()
                self.fail("!!!...PFT AppSvc didn't deploy within 30mins...!!!")
        self.log.info("Successfully deployed PFT AppSvc.")
    ...
    def test_authorization(self):
        ...
        ], expected_failure_code=404, expected_failure_error={
            "code": 404,
            ...
            if result.status_code == 429:
                ...
                result = (self.capellaAPI.cluster_ops_apis.
                          delete_free_tier_app_service(
                            ...
                            self.free_tier_cluster_id, self.free_tier_app_id,
                            header))
            self.validate_testcase(result, [202, 404], testcase, failures)
        if failures:
            for fail in failures:
                self.log.warning(fail)
    ...
    def test_query_parameters(self):
        ...
        for combination in self.create_path_combinations(
            ...
            result = self.capellaAPI.cluster_ops_apis.delete_free_tier_app_service(self.organisation_id, self.project_id, self.free_tier_cluster_id, 
                self.free_tier_app_id,
                **kwarg)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.delete_free_tier_app_service, (
                self.organisation_id, self.project_id,
                self.free_tier_cluster_id, self.free_tier_app_id
            )
        ]]
