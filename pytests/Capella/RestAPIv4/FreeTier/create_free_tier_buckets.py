...
class PostFreeTier(GetFreeTierBucket):

    def setUp(self, nomenclature="FreeTier_POST"):
        GetFreeTierBucket.setUp(self, nomenclature)
        self.flush_freetier_buckets(self.free_tier_cluster_id)
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.create_free_tier_bucket(
                organization, project, cluster, self.expected_res["name"],
                self.expected_res["memoryAllocationInMb"])
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = (self.capellaAPI.cluster_ops_apis.
                          create_free_tier_bucket(
    ...
    def test_authorization(self):
        ...
        for testcase in self.v4_RBAC_injection_init([
            ...
            result = self.capellaAPI.cluster_ops_apis.create_free_tier_bucket(
                ...
                self.expected_res["memoryAllocationInMb"], header)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = (self.capellaAPI.cluster_ops_apis.
                          create_free_tier_bucket(
    ...
    def test_query_parameters(self):
        ...
        for combination in self.create_path_combinations(
            ...
            result = self.capellaAPI.cluster_ops_apis.create_free_tier_bucket(self.organisation_id, self.project_id, self.free_tier_cluster_id,
                self.expected_res["name"],
                **kwarg)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
    ...
    def test_payload(self):
        ...
        for k in self.expected_res:
            for v in [
                ...
                testcase["description"] = "Testing `{}` with val: `{}` of {}"\
                    .format(k, v, type(v))
                if k == "memoryAllocationInMb" and v is None:
                    testcase["expected_error"] = {
                        "code": 6023,
                    ...
                    testcase["expected_status_code"] = 422
                elif k == "memoryAllocationInMb" and not isinstance(v, int):
                    testcase["expected_error"] = {
                        "code": 1000,
                        "hint": "The request was malformed or invalid.",
                        "httpStatusCode": 400,
                        "message": "Bad Request. Error: body contains incorrec"
                                   "t JSON type for field \"{}\".".format(k)
                    }
                    testcase["expected_status_code"] = 400
                elif k == 'memoryAllocationInMb':
                    if v < 100:
                        testcase["expected_error"] = {
                            "code": 6023,
                            "hint": "Cannot create bucket. The requested size "
                                    "of the bucket is less than the minimum "
                                    "amount of 100MB. Please choose a larger "
                                    "size for the bucket.",
                            "httpStatusCode": 422,
                            "message": "Cannot create bucket. The requested "
                                       "size of the bucket is less than the "
                                       "minimum amount of 100MB."
                        }
                        testcase["expected_status_code"] = 422
                    elif v > 1000:
                        testcase["expected_error"] = {
                            "code": 6002,
                            "hint": "The requested bucket size exceeds the "
                                    "maximum allowed size. Please choose a "
                                    "smaller size for the bucket.",
                            "httpStatusCode": 422,
                            "message": "Can not make requested modification "
                    ...
                    if not len(v) or v is None:
                        testcase["expected_error"] = {
                            "code": 6011,
                            "hint": "The bucket name is not valid. A "
                                    "bucket name cannot be empty. Please "
                                    "provide a valid name for the bucket.",
                            "httpStatusCode": 422,
                            "message": "The bucket name is not valid. A "
                        ...
                        testcase["expected_error"] = {
                            ...
                            "hint": "The bucket name provided is not valid. "
                                    ...
                                    " be 100 characters. Please provide a "
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.create_free_tier_bucket(
                        self.organisation_id, self.project_id,
                        self.free_tier_cluster_id,
                        testcase["name"], testcase["memoryAllocationInMb"])
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = (self.capellaAPI.cluster_ops_apis.
                          create_free_tier_bucket(
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.create_free_tier_bucket, (
                self.organisation_id, self.project_id,
                self.free_tier_cluster_id, self.expected_res["name"]
            )
        ]]
