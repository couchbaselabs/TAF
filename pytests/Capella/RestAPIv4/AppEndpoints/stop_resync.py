...
class DeleteResync(GetAppEndpoints):

    def setUp(self, nomenclature="Resync_Delete"):
        GetAppEndpoints.setUp(self, nomenclature)

    def tearDown(self):
        super(DeleteResync, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            appEndpointNa = self.appEndpointName

            if "url" in testcase:
                self.capellaAPI.cluster_ops_apis.deleteAppEndpointResync = \
                    testcase["url"]
            if "invalid_organizationId" in testcase:
                ...
                appEndpointNa = testcase["invalid_appEndpointName"]

            result = self.capellaAPI.cluster_ops_apis.stop_resync(
                organization, project, cluster, appService, appEndpointNa)
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.stop_resync, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id, self.appEndpointName
            )
        ]]
