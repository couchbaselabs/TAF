...
class PostAppEndpoints(GetAppEndpoints):
        ...
        for testcase in self.v4_RBAC_injection_init([
            ...
            if result.status_code == 429:
                ...
                result = self.capellaAPI.cluster_ops_apis.create_app_endpoint(
                    ...
                    self.expected_res["userXattrKey"],
                    header)
            if self.validate_testcase(result, [201, 412], testcase, failures):
                self.log.debug("Creation Successful")
    ...
    def test_query_parameters(self):
        ...
        for combination in self.create_path_combinations(
            ...
            if self.validate_testcase(result, [201], testcase, failures):
                self.log.debug("Creation Successful")
                # self.flush_appendpoints(self.project_id, self.cluster_id,
                #                     self.app_service_id, [result.json()["id"]])
    ...
    def test_payload(self):
        ...
        for testcase in testcases:
            ...
            if res.status_code == 201:
                self.log.debug("...VALID App Endpoint CREATE request...")
