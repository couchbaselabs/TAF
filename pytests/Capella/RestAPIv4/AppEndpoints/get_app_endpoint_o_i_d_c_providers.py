...
class GetOidcProviders(GetAppEndpoints):

    def setUp(self, nomenclature="ConnectOIDCProviders_GET"):
        ...
        self.log.info("Creating AppEndpoint OIDC Provider")
        ...
        if res.status_code != 201:
            ...
            self.tearDown()
            self.fail("Error while creating AppEndpoint.")
        self.log.info("AppEndpoint Oidc provider created successfully.")
        self.OIDCProvider_id = res.json()["providerId"]

    def tearDown(self):
        self.update_auth_with_api_token(self.curr_owner_key)
        # Delete the oidc provider and others
        raw_list = (self.capellaAPI.cluster_ops_apis.list_app_endpoint_o_i_d_c_providers(
            self.organisation_id, self.project_id, self.cluster_id,
            self.app_service_id, self.appEndpointName))
        list_of_providers = raw_list.json()["data"]
        if list_of_providers is not None and len(list_of_providers) > 1:
            for provider in list_of_providers:
                self.log.debug(provider)
                if provider["isDefault"]:
                    continue
                self.flush_oidcProviders(self.project_id, self.cluster_id,
                                         self.app_service_id,
                                         self.appEndpointName,
                                         provider["providerId"])

        self.log.info("Deleting the default AppEndpoint")
        self.flush_oidcProviders(self.project_id, self.cluster_id, self.app_service_id,
                                 self.appEndpointName, self.OIDCProvider_id)
        self.log.info("Successfully deleted the AppEndpoint.")

        super(GetOidcProviders, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.fetch_oidc_provider_info(
                organization, project, cluster, appService, appEndpointName, OIDCProvider)
    ...
    def test_query_parameters(self):
        self.log.debug(
            ...
            "appEndpointName: {}, OIDCProvider ID: {}".format(
                self.organisation_id, self.project_id, self.cluster_id,
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.fetch_app_endpoint_o_i_d_c_provider_info, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id, self.appEndpointName,
                self.OIDCProvider_id
            )
