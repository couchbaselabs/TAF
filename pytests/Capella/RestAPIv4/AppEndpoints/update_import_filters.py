...
class PutImportFilter(GetAppEndpoints):

    def setUp(self, nomenclature="ImportFilter_Put"):
        GetAppEndpoints.setUp(self, nomenclature)

        self.expected_res = ("function(doc) { "
                             "if (doc.type == 'mobile') { return true; } "
                             "}")
        # self.log.info("Creating ImportFilter")
        # res = self.capellaAPI.cluster_ops_apis.create_import_filter(
        ...
        #     self.tearDown()
        #     self.fail("Error while creating ImportFilter.")
        # self.log.info("ImportFilter created successfully.")

    def tearDown(self):
        self.update_auth_with_api_token(self.curr_owner_key)

        # # Delete the ImportFilter.
        # self.log.info("Deleting the ImportFilter")
        # res = self.capellaAPI.cluster_ops_apis.delete_import_filter(
        #     self.organisation_id, self.project_id, self.cluster_id,
        ...
        #     self.log.error("Result: {}".format(res.content))
        #     self.tearDown()
        #     self.fail("ImportFilter deletion failed")
        # self.log.info("Successfully deleted the ImportFilter.")

        super(PutImportFilter, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.update_import_filter(
                organization, project, cluster, appService,
                appEndpointKeyspace, self.expected_res)
    ...
    def test_authorization(self):
        ...
        for testcase in self.v4_RBAC_injection_init([
            ...
            result = self.capellaAPI.cluster_ops_apis.update_import_filter(
                self.organisation_id, self.project_id, self.cluster_id, 
                self.app_service_id, self.appEndpointName, self.expected_res,
                header)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.update_import_filter(
    ...
    def test_payload(self):
        ...
        for v in [
            ...
            testcase["expected_error"] = {
                ...
                "message": "bad request: 1 errors:\ncollection \"_default\" "
                           "import filter error: invalid javascript "
                           "syntax: ReferenceError: '{}' is not "
                           "defined".format(v)
            }
            testcase["expected_status_code"] = 400
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.update_import_filter(
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id, self.appEndpointName, testcase["payload"])
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.update_import_filter(
                    self.organisation_id, self.project_id, self.cluster_id,
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.update_import_filter, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id, self.appEndpointName
            )
        ]]
