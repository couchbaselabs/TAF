...
class GetCors(GetAppEndpoints):

    def setUp(self, nomenclature="GetCors"):
        GetAppEndpoints.setUp(self, nomenclature)

    def tearDown(self):
        super(GetCors, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.fetch_app_endpoint_c_o_r_s_info(
    ...
    def test_query_parameters(self):
        ...
        for combination in self.create_path_combinations(
            ...
            result = self.capellaAPI.cluster_ops_apis.fetch_app_endpoint_c_o_r_s_info(self.organisation_id, self.project_id, self.cluster_id, 
                self.app_service_id, self.appEndpointName,
                **kwarg)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.fetch_app_endpoint_c_o_r_s_info,(
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id, self.appEndpointName)
        ]]
