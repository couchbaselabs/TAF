...
class PutAccessControlFunction(GetAppEndpoints):

    def setUp(self, nomenclature="AccessControl_Put"):
        GetAppEndpoints.setUp(self, nomenclature)
        self.expected_res = ('function(doc,oldDoc,meta){channel(\'collection name\');}')

    def tearDown(self):
        super(PutAccessControlFunction, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            result = (self.capellaAPI.cluster_ops_apis
                      .update_access_control_function(
                        organization, project, cluster, appService,
                        appEndpointName, self.expected_res))
            ...
            if result.status_code == 429:
                ...
                result = (self.capellaAPI.cluster_ops_apis
                          .update_access_control_function(
                            ...
                            self.appEndpointName, self.expected_res, header))
            self.validate_testcase(result, [204], testcase, failures)
        ...
        for combination in self.create_path_combinations(
            ...
            result = (self.capellaAPI.cluster_ops_apis
                      .update_access_control_function(
                        ...
                        testcase["appEndpointName"], self.expected_res,
                        **kwarg))
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = (self.capellaAPI.cluster_ops_apis
    ...
    def test_payload(self):
        ...
        for v in [
            ...
            testcase = dict()
            testcase["payload"] = v
            testcase["desc"] = "Testing payload with val: `{}` of {}"\
                .format(v, type(v))
            testcase["expected_error"] = {
                "code": 400,
            ...
            if result.status_code == 429:
                ...
                result = (self.capellaAPI.cluster_ops_apis
                          .update_access_control_function(
                            self.organisation_id, self.project_id,
                            self.cluster_id, self.app_service_id,
                            self.appEndpointName, testcase["payload"]))
            self.validate_testcase(result, [204], testcase, failures,
                                   payloadTest=True)
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.update_access_control_function, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id, self.appEndpointName,
                self.expected_res
            )
