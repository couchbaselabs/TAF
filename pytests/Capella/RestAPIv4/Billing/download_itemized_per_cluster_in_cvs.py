...
class DownloadItemizedPerCluster(GetCluster):

    def setUp(self, nomenclature="DownloadItemizedPerCluster_POST"):
        GetCluster.setUp(self, nomenclature)

    def tearDown(self):
        super(DownloadItemizedPerCluster, self).tearDown()
    ...
    def test_api_path(self):
        testcases = [
            {
                ...
                "url": "/v3/organizations/{}/projects/{}/clusters/{}/billing/download",
        ...
        for testcase in testcases:
            ...
            if "url" in testcase:
                self.capellaAPI.cluster_ops_apis.billing_download_itemized_per_cluster_csv_endpoint = \
                    testcase["url"]
            if "invalid_organizationId" in testcase:
                organization = testcase["invalid_organizationId"]

            result = self.capellaAPI.cluster_ops_apis.get_download_itemized_per_cluster_csv_billing(
                organization, self.project_id, self.cluster_id)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.get_download_itemized_per_cluster_csv_billing(
                    organization, self.project_id, self.cluster_id)
            self.capellaAPI.cluster_ops_apis.billing_get_prepaid_credit_endpoint = \
                "/v4/organizations/{}/projects/{}/clusters/{}/billing/download"
    ...
    def test_authorization(self):
        ...
        for testcase in self.v4_RBAC_injection_init([
            ...
            result = self.capellaAPI.cluster_ops_apis.get_download_itemized_per_cluster_csv_billing(header)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.get_download_itemized_per_cluster_csv_billing(header)
    ...
    def test_query_parameters(self):
        ...
        for combination in self.create_path_combinations(
            ...
            result = self.capellaAPI.cluster_ops_apis.get_download_itemized_per_cluster_csv_billing(
                testcase["organizationID"], self.project_id, self.cluster_id, **kwarg)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.get_download_itemized_per_cluster_csv_billing(
                    testcase["organizationID"], self.project_id, self.cluster_id, **kwarg)
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.get_download_itemized_per_cluster_csv_billing, (
        ...
        for line in lines[1:]:
            ...
            for col in columns:
                ...
                if re.match(r'^-?\d+\.\d+$', col):
                    if not decimal_pattern.match(col):
                        return False
        return True

    def test_download_itemized_per_cluster_in_csv_date_ranges(self):
        today = datetime.now()
        date_fmt = "%Y-%m-%d"
        testcases = [
            {
    ...
    def test_download_itemized_per_cluster_in_csv_negative_dates(self):
        today = datetime.now()
        date_fmt = "%Y-%m-%d"
        testcases = [
            {
    ...
    def test_download_itemized_per_cluster_in_csv_individual_categories(self):
        today = datetime.now()
        date_fmt = "%Y-%m-%d"
        all_categories = [
    ...
    def test_download_itemized_per_cluster_in_csv_random_category_combos(self):
        today = datetime.now()
        date_fmt = "%Y-%m-%d"
        testcases = [
            {
    ...
    def test_download_itemized_per_cluster_in_csv_all_categories(self):
        today = datetime.now()
        date_fmt = "%Y-%m-%d"
        all_categories = [
            "operationalComputeAndStorage",
