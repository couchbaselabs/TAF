...
class GetDatabaseCredential(DatabaseCredentialBase):
    ...
    #                 testcase["expected_error"] = {
    #                     "code": 404,
    #                     "hint": "The requested database credential details "
    #                             "could not be found or fetched. Please ensure "
    #                             "that the correct user ID is provided.",
    #                     "message": "Database credential not found.",
    #                     "httpStatusCode": 404
    #                 }
    #         self.log.info("Executing test: {}".format(testcase["description"]))
    ...
    #             testcase["clusterID"], testcase["userID"], **kwarg)
    #         if result.status_code == 429:
    #             self.handle_rate_limit(int(result.headers["Retry-After"]))
    #             result = \
    #                 self.capellaAPI.cluster_ops_apis.fetch_database_user_info(
...
class UpdateDatabaseCredential(DatabaseCredentialBase):
    ...
    def test_authorization(self):
        ...
        for testcase in self.v4_RBAC_injection_init([
            ...
            result = self.capellaAPI.cluster_ops_apis.update_database_user(
                self.organisation_id, self.project_id, self.cluster_id,
                self.user_id, self.update_access, headers=header)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.update_database_user(
                    self.organisation_id, self.project_id, self.cluster_id,
