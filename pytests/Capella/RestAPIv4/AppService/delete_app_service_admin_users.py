...
class DeleteAdminUsers(GetAdminUsers):

    def setUp(self, nomenclature="AppService_DELETE"):
        GetAdminUsers.setUp(self, nomenclature)

    def tearDown(self):
        super(DeleteAdminUsers, self).tearDown()
    ...
    def test_api_path(self):
        ...
        failures = list()
        payload = copy.deepcopy(self.expected_res)
        payload["name"] = "user1"
        user_id = self.create_app_service_admin_user_to_be_tested(
            self.project_id,
            self.cluster_id,
            self.app_service_id,
            **payload)
        for testcase in testcases:
            ...
            if "url" in testcase:
                self.capellaAPI.cluster_ops_apis.app_service_admin_users = \
                    testcase["url"]
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.delete_app_service_admin_user, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id, self.user_id
            )
        ]]
