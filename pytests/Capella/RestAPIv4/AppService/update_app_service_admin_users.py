...
class PutAdminUsers(GetAdminUsers):

    def setUp(self, nomenclature="AppService_PUT"):
        GetAdminUsers.setUp(self, nomenclature)

    def tearDown(self):
        super(PutAdminUsers, self).tearDown()
    ...
    def test_api_path(self):
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.update_app_service_admin_user(
    ...
    def test_payload(self):
        testcases = list()
        for k in ["accessAllEndpoints", "endpoints"]:
            if k in []:
                continue
            ...
            for v in [
                ...
                testcase["description"] = "Testing `{}` with val: `{}` of {}"\
                    .format(k, v, type(v))
                if k == "endpoints" and type(v) == str and len(v) > 1:
                    testcase["expected_status_code"] = 400
                    testcase["expected_error"] = {
                        "code": 422,
                        "hint": "Please review your request and ensure that "
                                "all required parameters are correctly "
                                "provided.",
                        "message": "Payload for creating or modifying app "
                                   "service admin user contains invalid "
                                   "endpoint name(s).",
                        "httpStatusCode": 422
                    }
                else:
                    ...
                    testcase["expected_error"] = {
                        ...
                        "hint": "Please review your request and ensure that "
                                "all required parameters are correctly "
                                "provided.",
                        "message": "Payload for creating or modifying app service "
                                   "admin user contains or lacks both, list of "
                                   "endpoints and all endpoints flag.",
                        "httpStatusCode": 422
                    }
        ...
        for testcase in testcases:
            ...
            payload = {k:v for k, v in testcase.items() if k != "description"}
            self.log.debug("sending payload: {}".format(payload))
            result = self.capellaAPI.cluster_ops_apis.update_app_service_admin_user(
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id, self.user_id, **payload)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = self.capellaAPI.cluster_ops_apis.update_app_service_admin_user(
                    self.organisation_id, self.project_id, self.cluster_id,
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.update_app_service_admin_user, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id, self.user_id
            )
        ]]
