...
class DisableAppServiceLogStreaming(GetLogStreaming):

    def setUp(self, nomenclature="DisableAppServiceLogStreaming_DELETE"):
        GetLogStreaming.setUp(self, nomenclature)

    def tearDown(self):
        print("********** do nothing **********")
    #    super(DisableAppServiceLogStreaming, self).tearDown()

    def test_api_disable_app_service_log_streaming_path(self):
        testcases = [
            {
        ...
        for testcase in testcases:
            ...
            result = self.capellaAPI.cluster_ops_apis.disable_app_service_log_streaming(
                organization, project, cluster, appService)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = (self.capellaAPI.cluster_ops_apis.
                          disable_app_service_log_streaming(
                              organization, project, cluster, appService,
                            ))
            self.capellaAPI.cluster_ops_apis.disable_app_service_log_streaming_endpoint = \
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.disable_app_service_log_streaming, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id
            )
        ]]
