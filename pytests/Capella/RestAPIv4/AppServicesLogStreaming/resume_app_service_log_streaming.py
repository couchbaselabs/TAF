...
from pytests.Capella.RestAPIv4.AppServicesLogStreaming.pause_app_service_log_streaming import PauseAppServiceLogStreaming

class ResumeAppServiceLogStreaming(PauseAppServiceLogStreaming):

    def setUp(self, nomenclature="ResumeAppServiceLogStreaming_POST"):
        PauseAppServiceLogStreaming.setUp(self, nomenclature)

    def tearDown(self):
        print("********** do nothing **********")
    #    super(ResumeAppServiceLogStreaming, self).tearDown()

    def test_api_resume_app_service_log_streaming_path(self):
        testcases = [
            {
                "description": "Send call with valid path params"
            }, {
                "description": "Replace api version in URI",
                "url": "/v4/organizations/{}/projects/{}/clusters/{}/appservices/{}/logStreaming/activationState",
        ...
        for testcase in testcases:
            ...
            if "url" in testcase:
                self.capellaAPI.cluster_ops_apis.disable_app_service_log_streaming_endpoint = \
                    testcase["url"]
            ...
            result = self.capellaAPI.cluster_ops_apis.resume_app_service_log_streaming(
                organization, project, cluster, appService)
            if result.status_code == 429:
                self.handle_rate_limit(int(result.headers["Retry-After"]))
                result = (self.capellaAPI.cluster_ops_apis.
                          resume_app_service_log_streaming(
                              organization, project, cluster, appService,
                            ))
            self.capellaAPI.cluster_ops_apis.resume_app_service_log_streaming_endpoint = \
                "/v4/organizations/{}/projects/{}/clusters/{}/appservices/{}/"\
                "logStreaming/activationState"
    ...
    def test_multiple_requests_using_API_keys_with_same_role_which_has_access(
            self):
        api_func_list = [[
            self.capellaAPI.cluster_ops_apis.resume_app_service_log_streaming, (
                self.organisation_id, self.project_id, self.cluster_id,
                self.app_service_id
            )
        ]]
