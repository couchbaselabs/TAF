...
class OpsThroughputWorker(AbstractTimedThroughputWorker):

    def __init__(self, node):
        super(OpsThroughputWorker, self).__init__(
    ...
    def action(self, throughput):
        ...
        for _ in range(throughput):
            try:
                self.client.set(self.key, 0, 0, random_string(10))
            except Exception:
                success = False
