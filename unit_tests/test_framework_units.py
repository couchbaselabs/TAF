...
Run tests with: jython unit_tests/test_framework_units.py
"""
...
# Add paths for imports (same approach as testrunner.py)
sys.path = [".", "lib", "pytests", "pysystests", "couchbase_utils",
            "platform_utils", "connections", "constants"] + sys.path

# Python 2/3 compatible integer types check
try:
...
class TestCRC32(unittest.TestCase):
    ...
    def test_crc32_return_type(self):
        ...
        self.assertTrue(isinstance(result, integer_types))
...
class TestBucketUtils(unittest.TestCase):
    ...
    def test_get_vbucket_num_for_key(self):
        ...
        self.assertTrue(isinstance(vb3, integer_types))
    ...
    def test_get_vbucket_num_for_key_range(self):
        ...
        # Test multiple keys
        for i in range(100):
            vb = get_vbucket_num_for_key("key-%d" % i)
