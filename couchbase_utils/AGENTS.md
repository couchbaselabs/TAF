...
- Feature-specific utilities for Couchbase Server/Capella test automation.
- High-level helpers built on `cb_server_rest_util` + other framework components.
...
| `cloud_provider_utils/` | Cloud provider abstraction (AWS/Azure/GCP/Localstack) for cbbackupmgr/cbcontbk flags and object-store cleanup | [AGENTS.md](cloud_provider_utils/AGENTS.md) |
...
```
couchbase_utils/
├── cb_server_rest_util/    ← Pure REST wrappers (no business logic), used by all other utils
...
3. Create utility files, `snake_case` naming.
4. Add AGENTS.md with agent rules if complex logic exists.
5. Update this file (description + link).
