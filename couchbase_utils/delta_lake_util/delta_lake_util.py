...
class DeltaLakeUtils(object):
    ...
    def write_data_into_table(self, data, delta_table_path,
                              write_mode="append"):
        """
        ...
        table is present (create if not exists). Format s3_bucket_name/folder
        ...
        try:
            ...
            dataframe.write.format("delta").mode(write_mode).save(
                self.format_s3_path(delta_table_path))
            return True
        except Exception as err:
            self.log.error(str(err))
            return False

    def read_data_from_table(self, delta_table_path):
        """
        Reads Data from Delta lake table.
        :param delta_table_path: <str> full path on S3 where the delta lake
        table is present. Format s3_bucket_name/folder
    ...
    def stream_data_from_table(self, delta_table_path,
    ...
    def stream_data_into_json_file(self, delta_table_path, output_file_path):
        """
    ...
    def list_all_table_versions(self, delta_table_path):
        """
        ...
        """
        delta_table = DeltaTable.forPath(self.spark_session,
                                         self.format_s3_path(delta_table_path))
    ...
    def restore_previous_version_of_delta_table(self, delta_table_path,
                                                version_to_restore):
        """
    ...
    def perform_vacuum_on_delta_lake_table(self, delta_table_path,
                                           retention_time=0):
        """
    ...
    def delete_data_from_table(self, delta_table_path, delete_condition=None):
        """
    ...
    def update_data_in_table(self, delta_table_path,
