...
public class doc_ops {
	 ...
	 * @param documents List of Document documents to be upserted to the bucket
	 ...
	 * @return
	 */
	public List<HashMap<String, Object>> bulkUpsert(final Bucket bucket, List<Document> documents, final int persistTo, final int replicateTo, final long timeOut, final String timeUnit){
		final PersistTo persist = this.getPersistTo(persistTo);
		final ReplicateTo replicate = this.getReplicateTo(replicateTo);
		final TimeUnit time = this.getTimeUnit(timeUnit);
		List<HashMap<String, Object>> returnValues = Observable
				.from(documents)
				.flatMap(new Func1<Document, Observable<HashMap<String, Object>>>() {

					public Observable<HashMap<String, Object>> call(final Document docToInsert) {
						return bucket.async().upsert(docToInsert, persist, replicate, timeOut, time).flatMap(new Func1<Document, Observable<HashMap<String,Object>>>() {
							public Observable<HashMap<String, Object>> call(Document insertedDocument) {
	 ...
	 * @param documents List of Document docs to be inserted to the bucket
	 * @param replicateTo Replication constraint to watch
	 * @param persistTo Persistence constraint to watch
	 * @param timeOut Custom timeout
	 * @param timeUnit Unit for timeout
	 * @return
	 */
	public List<HashMap<String, Object>> bulkSet(final Bucket bucket, List<Document> documents, final int persistTo, final int replicateTo, final long timeOut, final String timeUnit){
		final PersistTo persist = this.getPersistTo(persistTo);
		final ReplicateTo replicate = this.getReplicateTo(replicateTo);
		final TimeUnit time = this.getTimeUnit(timeUnit);
		List<HashMap<String, Object>> returnValues = Observable
		...
		.flatMap(new Func1<Document, Observable<HashMap<String, Object>>>() {
			public Observable<HashMap<String, Object>> call(final Document docToInsert) {
				return bucket.async().insert(docToInsert, persist, replicate, timeOut, time).flatMap(new Func1<Document, Observable<HashMap<String,Object>>>() {
					public Observable<HashMap<String, Object>> call(Document insertedDocument) {
						HashMap<String, Object> returnValue = new HashMap<String, Object>();
						returnValue.put("Document", insertedDocument);
						returnValue.put("Error", null);
