...
This test plan covers **12 major test categories** with **53 individual test cases** designed to validate:
...
12. Fusion lifecycle management (enable/disable, hibernation, state machine)
...
**TAF Test Method:** `test_accelerator_creation_during_rebalance`
...
**TAF Test Method:** `test_accelerator_instance_type_validation`
...
**TAF Test Method:** `test_accelerator_termination_after_rebalance`
...
**TAF Test Method:** `test_accelerator_fallback_scenarios`
...
**TAF Test Method:** `test_accelerator_registration_and_download_completion`
...
**TAF Test Method:** `test_volume_creation_and_attachment_timing`
...
**TAF Test Method:** `test_volume_hydration_monitoring`
...
**TAF Test Method:** `test_volume_cleanup_verification`
...
**TAF Test Method:** `test_volume_iops_and_size_validation`
...
**TAF Test Method:** `test_volume_tagging_verification`
...
### B.6 Parallel Rebalance Slot Management

**Test Description:** Validate EBS volume slot management when multiple rebalances run in parallel. This covers both the per-host slot assignment and the control plane's enforcement of the 22-slot maximum — content previously duplicated in the removed L.1 test.

**TAF Test Method:** `test_parallel_rebalance_slot_management`
...
- `ErrMaximumVolumeSlotsUsed` returned when insufficient slots
- Parallel rebalances allowed only after initial rebalance completes slot validation

**Validation Steps:**
1. Initiate an initial rebalance to allocate slots
2. Attempt a parallel rebalance; verify slot validation passes when slots are available
3. Monitor per-host slot assignments for conflicts
4. Exhaust available slots and verify `ErrMaximumVolumeSlotsUsed` error is returned
5. Verify slot reclamation after rebalance completion
...
**TAF Test Method:** `test_add_nodes_with_fusion_enabled`
...
**TAF Test Method:** `test_remove_nodes_with_fusion_enabled`
...
**TAF Test Method:** `test_swap_rebalance_with_fusion`
...
**TAF Test Method:** `test_multi_node_scaling_parallel`
...
**TAF Test Method:** `test_disk_scaling_with_fusion`
...
**TAF Test Method:** `test_compute_scaling_with_fusion`
...
**TAF Test Method:** `test_combined_disk_compute_scaling`
...
**Expected Control Plane Behavior:**
- Combined scaling handled as a single control plane operation
- Fusion acceleration for data movement
- Changes applied sequentially by the control plane (disk resize and compute change are separate AWS API calls; the combined operation is not atomic at the AWS layer — validate each change independently)

**Validation Steps:**
1. Initiate combined scaling
2. Verify disk change applied
3. Verify compute change applied
4. Monitor accelerator lifecycle
5. Verify final configuration matches both requested changes

**Edge Cases:**
- Partial scaling failure (one succeeds, one fails)
...
**TAF Test Method:** `test_scaling_during_ongoing_operations`
...
**TAF Test Method:** `test_log_store_uri_discovery`
...
**TAF Test Method:** `test_log_store_size_consistency`
...
**TAF Test Method:** `test_log_store_object_lifecycle`
...
**TAF Test Method:** `test_s3_deletion_verification`
...
**TAF Test Method:** `test_accelerator_instance_failure`
...
**Fault Injection Mechanism (AWS FIS):**
- Use `awslib/fis_lib.py:create_compute_failure_experiment()` to stop or terminate an accelerator EC2 instance.
- Target instances by the `couchbase-cloud-function: fusion-accelerator` tag.
...
2. Induce failure via `fis_lib.py:start_experiment()` (instance stop/terminate)
...
**TAF Test Method:** `test_ebs_volume_attachment_failure`
...
**Test Description:** Validate behavior during network partition between accelerator instances and S3.

**TAF Test Method:** `test_network_partition_via_fis`
...
**Fault Injection Mechanism (AWS FIS):**
- Use `awslib/fis_lib.py` to create an AWS FIS experiment with the `aws:network:disrupt-connectivity` action targeting accelerator instance network interfaces, blocking outbound traffic to the S3 VPC endpoint.
- Alternatively, modify VPC Network ACLs programmatically via boto3 to drop traffic from accelerator subnets to S3 CIDR ranges for the duration of the test, then restore them.
- Verify the mechanism is in place before proceeding with validation steps.
...
**Validation Steps:**
1. Induce network partition using FIS experiment
2. Monitor accelerator retry behavior
3. Verify timeout handling and error logging
4. Restore connectivity; verify recovery after partition resolved
5. Verify rebalance either completes or fails cleanly (no hung state)
...
**Related Source Files:**
- TAF: `fusion_network_partition.py`, `awslib/fis_lib.py`
...
**TAF Test Method:** `test_az_failure_simulation_via_fis`
...
**Fault Injection Mechanism (AWS FIS):**
- Use `awslib/fis_lib.py:create_compute_failure_experiment()` scoped to a single AZ by filtering instances with `placement.availability-zone = <target-az>` and the fusion-accelerator tag.
- Use the AWS FIS `aws:ec2:asg-insufficient-instance-capacity-error` action targeting the accelerator Auto Scaling Groups in the target AZ to simulate capacity unavailability.
- Confirm all accelerator instances in the target AZ are in a stopped/terminated state before validating cross-AZ behavior.
...
**Validation Steps:**
1. Identify AZ containing accelerators
2. Simulate AZ failure via FIS experiment targeting that AZ
3. Verify accelerators in remaining AZs continue operation
4. Verify recovery after AZ restored
...
**TAF Test Method:** `test_graceful_rebalance_with_fusion`
...
5. Fetch rebalance report and parse timing breakdown; compare fusion mount time vs DCP backfill time using `fusion_monitor_util.py:log_rebalance_report()` to confirm fusion is providing measurable acceleration
...
**Related Source Files:**
- TAF: `fusion_sanity.py:test_fusion_rebalance()`, `fusion_volume.py`, `fusion_monitor_util.py:log_rebalance_report()`
...
**TAF Test Method:** `test_failover_rebalance_with_fusion`
...
**TAF Test Method:** `test_dcp_rebalance_with_fusion`
...
**TAF Test Method:** `test_rebalance_during_steady_state_workload`
...
**TAF Test Method:** `test_item_count_validation_after_rebalance`
...
**TAF Test Method:** `test_document_read_validation_after_rebalance`
...
**TAF Test Method:** `test_fusion_vs_nfs_storage_consistency`
...
**TAF Test Method:** `test_pending_bytes_monitoring`
...
**TAF Test Method:** `test_core_dump_detection`
...
**TAF Test Method:** `test_critical_error_scanning`
...
**TAF Test Method:** `test_hydration_failure_detection`
...
**TAF Test Method:** `test_recovery_from_transient_failures`
...
**TAF Test Method:** `test_manifest_creation_and_validation`
...
**Related Source Files:**
- TAF: `fusion_volume.py`, `fusion_cp_resource_monitor.py`; requires `FusionManifestUtil:get_manifest()`, `track_status_transitions()`, `verify_parts()`
...
**TAF Test Method:** `test_checkpoint_management`
...
**Related Source Files:**
- TAF: requires `FusionCheckpointUtil:get_checkpoint()`, `verify_recovery()`, `track_operations()`
...
**TAF Test Method:** `test_networking_vpc_endpoints_security_groups`
...
**Related Source Files:**
- TAF: requires `FusionNetworkingUtil:verify_vpc_endpoint()`, `verify_security_groups()`, `test_connectivity()`; also used by F.3 and F.4
- couchbase-cloud: `infra/aws/networking_vpc_endpoints.go`, `networking_security_groups.go`
...
**TAF Test Method:** `test_asg_cleanup_verification`
...
**TAF Test Method:** `test_invalid_rebalance_plan_handling`
...
**TAF Test Method:** `test_rate_limiting_and_throttling`
...
**TAF Test Method:** `test_large_data_volume_scaling`
...
4. Measure duration using `FusionScaleTestHelper:measure_duration()`
...
**Related Source Files:**
- TAF: `fusion_sanity.py:test_fusion_rebalance_at_scale()`; requires `FusionScaleTestHelper:measure_duration()`, `track_resource_usage()`, `calculate_metrics()`
...
**TAF Test Method:** `test_high_concurrency_rebalance`
...
3. Monitor resource usage using `FusionScaleTestHelper:track_resource_usage()`
4. Verify success
...
**Related Source Files:**
- TAF: `fusion_volume.py:test_volume_scaling()`; requires `FusionScaleTestHelper:track_resource_usage()`, `calculate_metrics()`
...
### L.1 Rebalance During Background Migration

**Test Description:** Validate behavior when operations occur during background migration.

**TAF Test Method:** `test_rebalance_during_background_migration`
...
### L.2 Tenant/Cluster Destruction During Rebalance

**Test Description:** Validate behavior when cluster is destroyed during active rebalance.

**TAF Test Method:** `test_cluster_destruction_during_rebalance`
...
## M. Fusion Lifecycle Management

### M.1 Enable Fusion on Existing Cluster

**Test Description:** Validate enabling fusion on a cluster that was created without fusion, verifying that the control plane provisions required infrastructure and the cluster enters a healthy "enabled" state.

**TAF Test Method:** `test_enable_fusion_on_existing_cluster`

**AWS Resources to Validate:**
- S3 bucket and VPC endpoint provisioned
- Security group rules updated for accelerator access
- Feature flag `ff-billing-fusion-enabled` state in control plane

**Expected Control Plane Behavior:**
- Fusion enable request triggers infrastructure provisioning
- Fusion status transitions: Disabled → Enabling → Enabled
- No active data movement triggered; cluster remains operational during enable
- Billing fixed-cost job begins charging for fusion on next hourly run

**Validation Steps:**
1. Create cluster with fusion disabled
2. Send enable-fusion request via control plane API
3. Monitor fusion status transitions using `fusion_monitor_util.py:wait_for_fusion_status(status="enabled")`
4. Verify S3 bucket and VPC endpoint exist in the cluster's AWS account
5. Verify security group rules updated
6. Verify next rebalance uses fusion acceleration

**Edge Cases:**
- Enable during ongoing rebalance (should queue or reject)
- Enable on cluster with insufficient disk type (non-GP3)
- Enable fails partway through (rollback behavior)

**Related Source Files:**
- TAF: `fusion_monitor_util.py:wait_for_fusion_status()`, `fusion_aws_util.py`; requires `FusionLifecycleUtil:enable_fusion()`, `get_fusion_status()`
- couchbase-cloud: `internal/clusters/fusion/`, `infra/aws/storage.go`, `infra/aws/networking_vpc_endpoints.go`

---

### M.2 Disable Fusion on Existing Cluster

**Test Description:** Validate disabling fusion on an active fusion-enabled cluster, verifying that accelerator infrastructure is torn down and billing stops.

**TAF Test Method:** `test_disable_fusion_on_existing_cluster`

**AWS Resources to Validate:**
- S3 VPC endpoint removed
- Security group rules restored to pre-fusion state
- No accelerator ASGs remaining

**Expected Control Plane Behavior:**
- Fusion disable waits for any in-progress rebalance to complete
- Fusion status transitions: Enabled → Disabling → Disabled
- Fixed billing job stops charging for fusion nodes after disable
- Background migration data returned to primary storage

**Validation Steps:**
1. Start with a fusion-enabled cluster in steady state
2. Send disable-fusion request via control plane API
3. Monitor fusion status transitions using `fusion_monitor_util.py:wait_for_fusion_status(status="disabled")`
4. Verify VPC endpoint removed from the cluster's VPC
5. Verify no orphaned accelerator instances or ASGs
6. Verify next rebalance does not create accelerator instances

**Edge Cases:**
- Disable during active fusion rebalance (should block or cancel rebalance)
- Disable with pending background migration data
- Re-enable immediately after disable

**Related Source Files:**
- TAF: `fusion_monitor_util.py:wait_for_fusion_status()`, `fusion_cp_resource_monitor.py`; requires `FusionLifecycleUtil:disable_fusion()`, `get_fusion_status()`
- couchbase-cloud: `internal/clusters/fusion/`, `infra/aws/destroyer.go`

---

### M.3 Fusion Status State Machine Validation

**Test Description:** Exhaustively validate all fusion status state transitions to ensure the state machine enforces valid progression and rejects invalid transitions.

**TAF Test Method:** `test_fusion_status_state_machine`

**Expected State Transitions (valid):**
- `Disabled` → `Enabling` (on enable request)
- `Enabling` → `Enabled` (on successful provisioning)
- `Enabled` → `Disabling` (on disable request)
- `Disabling` → `Disabled` (on successful teardown)
- `Enabling` → `Disabled` (on provisioning failure/rollback)

**Expected Rejected Transitions (invalid):**
- `Disabled` → `Disabling` (cannot disable what is already disabled)
- `Enabling` → `Enabled` → `Enabling` without `Disabling` in between
- Any transition during an in-progress rebalance that conflicts with the requested change

**Validation Steps:**
1. Map out current fusion status via `fusion_monitor_util.py:wait_for_fusion_status()`
2. Trigger each valid transition in sequence and verify the resulting state
3. Attempt each invalid transition and verify the control plane rejects it with an appropriate error
4. Verify the status is accurately reflected in both the control plane API response and the cluster's internal state

**Edge Cases:**
- Concurrent enable and disable requests
- Status stuck in transitional state (Enabling/Disabling) due to infrastructure failure
- State machine recovery after control plane restart

**Related Source Files:**
- TAF: `fusion_monitor_util.py:wait_for_fusion_status()`; requires `FusionLifecycleUtil:enable_fusion()`, `disable_fusion()`, `get_fusion_status()`
- couchbase-cloud: `internal/clusters/fusion/`, `internal/clusters/fusion/nodes/`

---

### M.4 Hibernation with Fusion Enabled

**Test Description:** Validate that a fusion-enabled cluster can be hibernated and resumed without data loss, and that fusion infrastructure is correctly suspended and restored across the hibernation lifecycle.

**TAF Test Method:** `test_hibernation_with_fusion_enabled`

**AWS Resources to Validate:**
- EC2 instances stopped during hibernation
- EBS guest volumes detached and available (not deleted) during hibernation
- Accelerator ASGs scaled to zero during hibernation
- S3 VPC endpoint and bucket remain intact during hibernation

**Expected Control Plane Behavior:**
- Hibernation waits for any in-progress rebalance to complete before hibernating
- Guest volumes are detached but preserved (not deleted) during hibernation
- Fusion status remains "enabled" throughout hibernation
- On resume, cluster nodes restart and fusion-managed storage is intact
- First rebalance after resume uses fusion acceleration normally

**Validation Steps:**
1. Enable fusion on cluster and complete at least one fusion rebalance
2. Hibernate the cluster via control plane API
3. Verify all EC2 instances are stopped
4. Verify guest volumes remain in `available` state (not deleted)
5. Verify no active accelerator ASGs exist during hibernation
6. Resume the cluster
7. Verify fusion status is still "enabled" after resume
8. Initiate a new rebalance and verify fusion acceleration activates

**Edge Cases:**
- Hibernate during ongoing background migration
- Hibernate with guest volumes actively in use
- Resume failure leaving cluster in partial state
- Extended hibernation (>24h) and volume availability

**Related Source Files:**
- TAF: `fusion_monitor_util.py:wait_for_fusion_status()`, `fusion_aws_util.py:list_accelerator_instances()`; requires `FusionLifecycleUtil:hibernate_cluster()`, `resume_cluster()`, `get_fusion_status()`
- couchbase-cloud: `internal/clusters/fusion/`, `infra/aws/volumes.go`, `accelerator/accelerator.go`
...
3. A.5 - Accelerator Registration and Download Completion
4. B.1 - Volume Creation and Attachment Timing
5. B.2 - Volume Hydration Monitoring
6. B.3 - Volume Cleanup Verification
7. C.1 - Add Nodes with Fusion Enabled
8. G.1 - Graceful Rebalance with Fusion
9. H.1 - Item Count Validation After Rebalance
10. I.1 - Core Dump Detection
11. M.1 - Enable Fusion on Existing Cluster
12. M.2 - Disable Fusion on Existing Cluster
13. M.3 - Fusion Status State Machine Validation

### Priority 2 - Core Functionality
1. A.4 - Accelerator Fallback Scenarios
2. C.2 - Remove Nodes with Fusion Enabled
3. D.1 - Disk Scaling with Fusion
4. E.1 - Log Store URI Discovery
5. J.1 - Manifest Creation and Validation
6. J.4 - ASG Cleanup Verification
7. M.4 - Hibernation with Fusion Enabled

### Priority 3 - Edge Cases and Resilience
1. F.1 - Accelerator Instance Failure
2. F.2 - EBS Volume Attachment Failure
3. F.3 - Network Partition via FIS
4. F.4 - AZ Failure Simulation via FIS
5. J.5 - Invalid Rebalance Plan Handling
6. J.6 - Rate Limiting and Throttling
7. B.6 - Parallel Rebalance Slot Management
...
| Utility | Purpose | Required Methods | Used By |
|---------|---------|------------------|---------|
| FusionManifestUtil | Manifest tracking | `get_manifest()`, `track_status_transitions()`, `verify_parts()` | J.1 |
| FusionCheckpointUtil | Checkpoint management | `get_checkpoint()`, `verify_recovery()`, `track_operations()` | J.2 |
| FusionNetworkingUtil | Networking validation | `verify_vpc_endpoint()`, `verify_security_groups()`, `test_connectivity()` | J.3, F.3, F.4 |
| FusionScaleTestHelper | Scale test utilities | `measure_duration()`, `track_resource_usage()`, `calculate_metrics()` | K.1, K.2 |
| FusionLifecycleUtil | Lifecycle management | `enable_fusion()`, `disable_fusion()`, `get_fusion_status()`, `hibernate_cluster()`, `resume_cluster()` | M.1, M.2, M.3, M.4 |
...
7. Fusion status remains "enabled" during operations (unless a lifecycle test is explicitly changing it)
...
4. Rebalance duration improvement measurable (confirm via rebalance report: fusion mount time < DCP backfill time for equivalent data)
...
- `docs/test-plans/fusion_backup_restore_test_plan.md` - Fusion backup and restore test plan (covers S3 snapshot, backup scheduling, and recovery scenarios — complementary to this functional test plan)
...
| 1.1 | 2026-04-29 | Droid | Fixed test count (51→53) and category count (10→12); added M section (enable/disable, state machine, hibernation); merged duplicate L.1 into B.6; promoted A.5 and B.2 to Priority 1; added M tests to Priority 1/2; specified AWS FIS fault injection mechanism for F.3/F.4; corrected D.3 atomicity claim; folded K.3 rebalance report analysis into G.1 validation; added Python test method names to all tests; tied required new utilities to specific test cases; added cross-reference to fusion_backup_restore_test_plan.md |
