...
#    \cp -rf $PWD/productivitynautomation/ansible_setup/hosts /root/cloud/hosts
  ...
  find "$WORKSPACE/logs" -mindepth 1 -maxdepth 1 -type d -ctime +30 -exec rm -rf {} \;

  # Reclaim whole workspaces of jobs that haven't run in 90+ days, node-wide.
  ...
  for base in /data/workspace /root/workspace; do
    [ -d "$base" ] || continue
    find "$base" -mindepth 2 -maxdepth 2 -type d -name logs -ctime +90 2>/dev/null | while IFS= read -r logdir; do
