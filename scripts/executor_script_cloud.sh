...
if [ "$server_type" = "SERVERLESS_COLUMNAR" ]; then
  ...
  cluster_info="{\"pod\": \"$capella_api_url\", \"tenant_id\": \"$tenant_id\", \"capella_user\": \"$capella_user\", \"capella_pwd\": \"$capella_password\", \"region\": \"$capella_region\", \"project\": \"$project_id\", \"override_token\": \"$override_token\", \"columnar_image\": \"$columnar_image\", \"override_key\": \"$override_key\",}"
