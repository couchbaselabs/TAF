        ...
        try:
            ...
            if info.type.lower() != 'windows':
                ...
                core_files.extend(remote.file_ends_with(
                    ...
                    ".dmp"))
                if core_files:
                    print "found crashes on {0}: {1}".format(info.ip,
                                                             core_files)
                else:
                    ...
                    if core_file.find('erl_crash.dump') != -1:
                        ...
                        if remote.get_file(remote_path, file_name,
                                           os.path.join(self.path,
                                                        erl_crash_file_name)):
                            print 'downloaded core file : {0}'.format(
                                core_file)
                            i += 1
                        ...
                        if remote.get_file(remote_path, file_name,
                            ...
                            print 'downloaded breakpad .dmp file : {' \
                                  '0}'.format(
                                core_file)
                            i += 1
                    else:
                        ...
                        if remote.get_file(remote_path, file_name,
                                           os.path.join(self.path,
                                                        core_file_name)):
                            print 'downloaded core backtrace : {' \
                                  '0}'.format(
                                core_log_output)
                            i += 1
                if i > 0:
                    ...
                    print "put all crashes on {0} in backup folder: " \
...
class Clearcoredumps(object):
    ...
    def run(self):
        ...
        try:
            ...
            if info.type.lower() != 'windows':
                core_files = []
                print "looking for Erlang/Memcached crashes on {0} " \
                      "... ".format(
                    info.ip)
                core_files.extend(remote.file_starts_with(
                    "/opt/{0}/var/lib/{0}/".format(server_type),
                ...
                core_files.extend(remote.file_ends_with(
                    ...
                    ".dmp"))
                if core_files:
                    print "found dumps on {0}: {1}".format(info.ip,
                                                           core_files)
                    command = "mkdir -p /tmp/backup_crash/{0};" \
                              "mv -f /tmp/core* /tmp/backup_crash/{" \
                              "0};" \
                    ...
                    for core_file in core_files:
                        ...
                        if remote.delete_file(remote_path, file_name):
                            print 'deleted core file : {0}'.format(
                                core_file)
                    remote.disconnect()
                else:
