...
# Use jython from env if available, else check common locations
JYTHON_HOME := $(or $(wildcard /opt/homebrew/bin/jython),$(wildcard /usr/local/bin/jython),$(wildcard /opt/jython/bin/jython))
JYTHON := $(if $(JYTHON_HOME),$(JYTHON_HOME),jython)

.PHONY: clean TAF test unit_test
...
# Run unit tests without requiring external cluster
unit_test:
	$(JYTHON) unit_tests/test_framework_units.py
