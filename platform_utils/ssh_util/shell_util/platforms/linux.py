...
class Linux(ShellConnection, LinuxConstants):
    ...
    def restore_partition(self, location, original_device=None):
        ...
        command = "losetup -j {0} | cut -d: -f1".format(backing_file)
        output, error = self.execute_command(command)
        self.log_command_output(output, error)
        loop_device = output[0].strip() if output and output[0].strip() else None
        ...
        command = "rm -f {0} /usr/disk-img/disk-quota.ext4".format(backing_file)
        output, error = self.execute_command(command)
        self.log_command_output(output, error)
