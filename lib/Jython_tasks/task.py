...
class Task(Callable):
    ...
    def __str__(self):
        if self.exception:
            ...
            log.info("Task %s completed on: %s"%(self.thread_name, str(time.strftime("%H:%M:%S", time.gmtime(self.end_time)))))
...
class GenericLoadingTask(Task):
    def __init__(self, cluster, bucket, client, batch_size=1, pause_secs=1, timeout_secs=60, compression=True, throughput_concurrency=4):
    ...
    def _create_batch_client(self, key_val, shared_client = None):
        ...
        except (self.client.MemcachedError, ServerUnavailableException, socket.error, EOFError, AttributeError, RuntimeError) as error:
            self.set_exception(error)
...
class LoadDocumentsTask(GenericLoadingTask):
    ...
    def __init__(self, cluster, bucket, client, generator, op_type, exp, flag=0,
        ...
        super(LoadDocumentsTask, self).__init__(cluster, bucket, client, batch_size=batch_size,
                                    ...
                                    timeout_secs=timeout_secs, compression=compression, throughput_concurrency=throughput_concurrency)
    ...
    def call(self):
        ...
        if not self._compare(self.comparison, str(stat_result), self.value):
            log.warn("Not Ready: %s %s %s %s expected on %s, %s bucket" % (self.stat, stat_result,
                      self.comparison, self.value, self._stringify_servers(), self.bucket.name))
            time.sleep(5)
            self.call()
            return
