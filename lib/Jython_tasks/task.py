...
class MonitorClusterStatsTask(Task):
    def __init__(self, cluster, monitor_stats=list(), period=1, timeout=None, store_csv=False, csv_dir=None):
        super(MonitorClusterStatsTask, self).__init__(f"StatsMonitorTask{cluster.name}_{time.time()}")
        self.cluster = cluster
        self.sleep = period
        self.monitor_stats = monitor_stats
        self.stop_task = False
    ...
    def complete_task(self):
        ...
        self.log.info(f"Thread '{self.thread_name}' completed")

        if self.store_csv:
            for stat in self.result:
                csv_path = os.path.join(self.csv_dir, stat + ".csv")
                self._generate_csv(self.result[stat], csv_path)
                self.test_log.info(f"CSV file for the monitored stat {stat} generated at {csv_path}")
    ...
    def _generate_csv(self, data, csv_path):
        ...
        if len(sorted_columns) != 0:
            df = pd.DataFrame(flattened_data, columns=["Time"] + sorted_columns)
    ...
    def call(self):
        ...
        while not self.stop_task or (end != None and end > time.time()):
            try:
                ...
                self.log.error(f"Exception occurred during : {str(e)}")
                self.set_exception(e)
