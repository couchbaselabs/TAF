...
class Link(object):
    ...
    def __init__(self, name=None, dataverse_name="Default", properties={}):
        ...
        self.properties["scope"] = CBASHelper.unformat_name(self.dataverse_name)
...
class Dataset(object):
    ...
    def __init__(self, name="cbas_ds", dataverse_name="Dafault",
                 link_name=None, dataset_source="internal", dataset_properties={},
        ...
        if self.dataset_source == "internal":
            ...
            if self.kv_collection:
                self.full_kv_entity_name = self.get_fully_qualified_kv_entity_name(cardinality=3)
            else:
                self.full_kv_entity_name = self.get_fully_qualified_kv_entity_name(cardinality=1)
    ...
    def get_fully_qualified_kv_entity_name(self, cardinality=1):
        if cardinality == 1:
            ...
            return CBASHelper.format_name(self.kv_bucket.name, self.kv_scope.name)
        elif cardinality == 3:
            return CBASHelper.format_name(self.kv_bucket.name, self.kv_scope.name, self.kv_collection.name)
...
class CBAS_Collection(Dataset):
    ...
    def __init__(self, name="cbas_ds", dataverse_name="Dafault",
        ...
        super(CBAS_Collection,self).__init__(
            name, dataverse_name, link_name, dataset_source, dataset_properties,
...
class Synonym(object):
    ...
    def __init__(self, name, cbas_entity_name, cbas_entity_dataverse, dataverse_name="Default",
        ...
        self.cbas_entity_dataverse = CBASHelper.format_name(cbas_entity_dataverse)
        self.dataverse_name = CBASHelper.format_name(dataverse_name)
        self.full_name = CBASHelper.format_name(self.dataverse_name, self.name)
        self.cbas_entity_full_name = CBASHelper.format_name(self.cbas_entity_dataverse, self.cbas_entity_name)
...
class CBAS_Index(object):
    ...
    def __init__(self, name, dataset_name, dataverse_name, indexed_fields=None):
        '''
        ...
        self.dataverse_name = CBASHelper.format_name(dataverse_name)
        self.full_dataset_name = CBASHelper.format_name(self.dataverse_name, self.dataset_name)
