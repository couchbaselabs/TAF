...
class SDKClient(object):
    ...
    def __init__(self, bucket, hosts=["localhost"], scheme="couchbase",
                 uhm_options=None, password=None,
    ...
    def _createConn(self):
        try:
            ...
            self.cluster = Cluster.connect(ClusterEnvironment
                                           .builder(", ".join(self.hosts).replace(" ", ""), "Administrator", "password")
...
#             self.cluster.disconnect()
    ...
    def delete(self, key, timeout=120, timeoutunit=TimeUnit.SECONDS, ttl=0, ttlunit=TimeUnit.DAYS, durability=DurabilityLevel.MAJORITY, max_retry=3):
        retry_count = 0
        retry = True
        while retry_count <= max_retry and retry:
            try:
                ...
                print("Error during remove of {0} - {1}"
                          .format(key, e))
                retry = False
    ...
    def insert(self, key, value, timeout=120, timeoutunit=TimeUnit.SECONDS, ttl=0, ttlunit=TimeUnit.DAYS, durability = DurabilityLevel.MAJORITY):
        success = dict()
        fail = dict()
        ...
        try:
            ...
            success.update({"key":key, "value":content,"error": None, "status":True})
            print insertResult
        except DocumentAlreadyExistsException as ex:
            print("The document already exists! => " + str(ex));
            fail.update({"key":key, "value":content,"error": str(ex), "status":False})
    ...
    def read(self, key):
        ...
        if getResult.isPresent():
            print("Found document: cas=%s, content=%s"%(str(getResult.get().cas()), str(getResult.get().contentAsObject())))
            success.update({"key":key, "value":str(getResult.get().contentAsObject()),"error": None, "status":True})
        else:
            print("Document not found!");

    def upsert(self, key, value, timeout=120, timeoutunit=TimeUnit.SECONDS, ttl=0, ttlunit=TimeUnit.DAYS, durability = DurabilityLevel.MAJORITY):
        success = dict()
        fail = dict()
        content = self.__translate_to_json_object(value)
        try:
    ...
    def insert_multi(self, keys, ttl=None, ttlunit=None,
                  timeOut=10, timeUnit="seconds", retry=5, 
                  doc_type="json", durability=DurabilityLevel.MAJORITY):
        ...
        while retry > 0:
            for doc in docs:
                s, f = self.insert(doc["key"], doc["value"], timeOut, timeUnit, ttl, ttlunit, durability)
                if s:
                    success[doc["key"]] = s
                if f:
                    fail[doc["key"]] = f
    ...
    def upsert_multi(self, keys, ttl=None, ttlunit=None,
                  timeOut=10, timeUnit="seconds", retry=5, 
                  doc_type="json", durability=DurabilityLevel.MAJORITY):
        ...
        while retry > 0:
            for doc in docs:
                s, f = self.upsert(doc["key"], doc["value"], timeOut, timeUnit, ttl, ttlunit, durability)
                if s:
                    success[doc["key"]] = s
                if f:
                    fail[doc["key"]] = f
...
# print client.upsert("dh_persist_key", '{"test":"value2"}', ttl=1, ttlunit=TimeUnit.MINUTES)
# client.read("dh_persist_key")
...
doc_create = doc_generator("ritesh", 0, 10000,
                                   doc_size=10,
                                   doc_type="json",
                                   target_vbucket=None,
                                   vbuckets=1024)
